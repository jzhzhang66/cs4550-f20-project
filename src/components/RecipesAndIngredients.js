...
const RecipesAndIngredients = (
  { mealPlan, dailyPlanId, lessonId, recipes= [{spoonRecipeId:""}], ingredients, deleteIngredient, deleteRecipe }) =>
      ...
      <ul>
        {
          recipes.map(recipe => <li key={recipe.id}>
            <Link to={`/recipes/${recipe.spoonRecipeId}`}>
              {recipe.servingUnit}
            </Link>
      ...
      <ul>
        {
          ingredients.map(ingredient => <li key={ingredient.id}>
            <Link to={`/ingredients/${ingredient.spoonIngredientId}`}>
